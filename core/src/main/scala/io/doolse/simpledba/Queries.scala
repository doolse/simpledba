...
trait WriteQueries[F[_], T] {
  ...
  def replaceAll[K](f: T => K): Pipe2[F, T, T, WriteOp] = (exstream, newstream) => for {
    exmap <- exstream.scan(Map.empty[K, T])((m,t) => m.updated(f(t), t))
    res <- newstream.scan((Stream.empty : Stream[F, WriteOp], exmap)) {
      (m, t) =>
        val k = f(t)
        val ex = exmap.get(k)
        ex match {
  ...
  def insertUpdateOrDelete(existing: Option[T], newvalue: Option[T]): Stream[F, WriteOp] = (existing, newvalue) match {
