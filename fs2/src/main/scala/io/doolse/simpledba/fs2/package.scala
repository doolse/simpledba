...
package object fs2 {
  ...
  implicit def fs2StreamEffects[F[_]](implicit S: Sync[F]): Streamable[Stream[F, *], F] = new Streamable[Stream[F, *], F] {
      ...
      override def maxMapped[A, B](n: Int, s: Stream[F, A])(f: Seq[A] => B): Stream[F, B] =
        s.chunkN(n).map(c => f(c.toVector))

      override def read1[A](s: Stream[F, A]): F[A] = S.flatMap(s.compile.toList)(
        _.headOption.fold[F[A]](S.raiseError(new Throwable("Expected a single result")))(S.pure[A]))

      override def flatMapS[A, B](fa: Stream[F, A])(fb: A => Stream[F, B]): Stream[F, B] = fa.flatMap(fb)

      override def mapS[A, B](fa: Stream[F, A])(f: A => B): Stream[F, B] = fa.map(f)
