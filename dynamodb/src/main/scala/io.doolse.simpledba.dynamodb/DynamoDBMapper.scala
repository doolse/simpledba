...
case class TableBuilder[T, TRec, ColRec <: HList](
  ...
  def partKeys[Out <: HList, PKL <: HList](cols: Cols[PKL])(
    ...
    val derivedKey = KeyAttribute
      .mapped[SdkBytes, Out](KeyType.HASH, DynamoDBMapper.derivedPK.value.name, DynamoDBColumn.bytesCol,
        CompositeKeyBuilder.toSdkBytes[Out])
    PartKeyTable(
      name, derivedKey,
...
case class PartKeyTable[T0, ColRec <: HList, PK0, Indexes0 <: HList](
  ...
  def derivedSortKey[A](f: T0 => A, attrName: String = "derivedSK")(implicit ev: DynamoDBPKColumn[A], col: DynamoDBColumn[A]): FullKeyTable[T0, ColRec, PK0, A, Indexes0] = {
  ...
  def sortKeys[Out <: HList, SKL <: HList](cols: Cols[SKL])(
    ...
    val derivedKey = KeyAttribute
      .mapped[SdkBytes, Out](KeyType.RANGE, DynamoDBMapper.derivedSK.value.name, DynamoDBColumn.bytesCol, out => {
    ...
    }, (t, rec) => derivedColumns(t, rec) :+ derivedKey.toNamedValue(toOut(rec)), localIndexes)
  }
