...
case class DynamoDBScalar[T](
                              ...
                              range: (T, T)) extends DynamoDBSortableColumn[T]

trait DynamoDBColumnLP2 {
  implicit def isoColumn[A, B](implicit iso: PartialIsoAtom[A, B], oc: DynamoDBScalar[B]): DynamoDBScalarColumn[A] = new DynamoDBScalarColumn[A] {
...
trait DynamoDBColumnLP extends DynamoDBColumnLP2 {
  implicit def rangedIsoScalar[A, B](implicit iso: RangedAtom[A, B], oc: DynamoDBScalar[B]): DynamoDBScalar[A] =
    DynamoDBScalar[A](oc.from.andThen(_.map(iso.to)),
      oc.to.compose(iso.from),
      (o: A, n: A) => oc.diff(iso.from(o), iso.from(n)),
      iso.defaultValue,
      oc.attributeType,
      iso.from.andThen(oc.sortablePart), iso.range
    )

  implicit def isoScalar[A, B](implicit iso: IsoAtom[A, B], oc: DynamoDBScalar[B]): DynamoDBScalar[A] =
    DynamoDBScalar[A](oc.from.andThen(_.map(iso.to)),
      iso.from andThen oc.to,
      (o: A, n: A) => oc.diff(iso.from(o), iso.from(n)),
      iso.to(oc.default),
      oc.attributeType,
      oc.sortablePart compose iso.from, {
        val (l, r) = oc.range
        (iso.to(l), iso.to(r))
      })
}
...
object DynamoDBColumn extends DynamoDBColumnLP {
  ...
  = DynamoDBScalar[T](_.map(_from), _to, putUpdate(_to), _default, attr, _sortablePart, _range)
  ...
  private def fixEmpty(s: Set[String]) = if (s.isEmpty) Set(EmptyStringSetValue) else s
  ...
  implicit def isoSet[A](implicit iso: PartialIsoAtom[A, String]) = new DynamoDBColumn[Set[A]] {
    ...
    val to = m => new AttributeValue().withSS(fixEmpty(m.map(iso.from)).asJava)
  }
