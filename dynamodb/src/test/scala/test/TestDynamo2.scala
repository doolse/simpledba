...
import io.doolse.simpledba.dynamodb.{DynamoDBColumn, DynamoDBMapper2, DynamoDBRelationIO}
import shapeless._
...
object TestDynamo2 extends App {
  ...
  implicit val embeddedColumnMapper = GenericColumnMapper[EmbeddedFields]
  ...
  val mappedTable = mapper.relation[Inst]("institution").keys('uniqueid, 'enabled)
  val qpk = mappedTable.queryByKey
  println(qpk)
  println(mappedTable)
}
