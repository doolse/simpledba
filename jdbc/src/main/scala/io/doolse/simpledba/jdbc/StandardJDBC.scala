...
object StandardJDBC {
  ...
  abstract class StandardSchemaSQL(config: JDBCConfig) extends JDBCSchemaSQL
  {
    ...
    def createTable(t: TableDefinition): String = {
      val colStrings = t.columns.map { cb =>
        s"${col(cb)} ${config.typeName(cb.columnType, t.primaryKey.contains(cb.name))}${if (!cb.columnType.nullable) " NOT NULL" else ""}"
      }
    ...
    def addColumns(t: TableColumns): Seq[String] = {
      def mkAddCol(cb: NamedColumn) = s"ADD COLUMN ${col(cb)} ${config.typeName(cb.columnType, false)}"
    ...
    override def createIndex(t: TableColumns, named: String): String =
      s"CREATE INDEX ${config.escapeColumnName(named)} on ${config.escapeTableName(t.name)} ${t.columns.map(col).mkString("(", ",", ")")}"
  }
  ...
  def stdExpressionSQL(c: JDBCConfig)(expr: SQLExpression): String = {
    expr match {
      ...
      case FunctionCall(name, params) => s"$name${params.map(stdExpressionSQL(c)).mkString("(", ",", ")")}"
  ...
  def stdOrderBy(c: JDBCConfig, oc: Seq[(NamedColumn, Boolean)]): String = {
    def orderClause(t: (NamedColumn, Boolean)) = s"${c.escapeColumnName(t._1.name)} ${if (t._2) "ASC" else "DESC"}"
  ...
  def stdSQLQueries(mc: JDBCConfig)(q: JDBCPreparedQuery): String = {
    ...
    q match {
      ...
      case JDBCInsert(t, c) =>
        s"INSERT INTO ${mc.escapeTableName(t)} ${brackets(c.map(v => escapeCol(v.column)))} VALUES ${brackets(c.map(v =>
          expr(v.expression)))}"
      case JDBCDelete(t, w) =>
