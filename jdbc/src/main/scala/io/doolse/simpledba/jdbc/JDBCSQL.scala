...
case class JDBCUpdate(table: String, assignments: Seq[ColumnExpression], where: Seq[JDBCWhereClause]) extends JDBCPreparedQuery

case class JDBCDelete(table: String, where: Seq[JDBCWhereClause]) extends JDBCPreparedQuery

case class JDBCSelect(table: String, columns: Seq[SQLProjection], where: Seq[JDBCWhereClause],
                      ordering: Seq[(NamedColumn, Boolean)], limit: Boolean) extends JDBCPreparedQuery
