...
case class JDBCRelation[C[_] <: JDBCColumn, T, R <: HList](name: String, sqlMapping: JDBCConfig, all: Columns[C, T, R]) {
  def edit[A](k: Witness, col: C[A])(implicit css: ColumnSubsetBuilder.Aux[R, k.T :: HNil, A :: HNil]): JDBCRelation[C, T, R] = {
  ...
  def key(k: Witness)(implicit css: ColumnSubsetBuilder[R, k.T :: HNil]): JDBCTable.Aux[C, T, R, css.Out, k.T :: HNil] = {
    val (keys, toKey) = all.subset[k.T :: HNil]
    JDBCTable(name, sqlMapping, all, css, toKey)
  }

  def keys[K <: HList](k: Cols[K])
...
trait JDBCTable[C[_] <: JDBCColumn]
{
  ...
  def writes: WriteQueries[JDBCIO, Data] = new WriteQueries[JDBCIO, Data] {
    ...
    override def insertAll: Pipe[JDBCIO, Data, WriteOp] = _.map {
      t =>
        ...
        JDBCWriteOp(insertQuery, config, bindParameters(columnBinders.columns.map(_._1._1)).map(v => Seq(ValueLog(v))))
    }
      ...
      case (o, n) =>
        ...
        if (oldKey != keyVal) {
          ...
          if (updateColumns.columns.isEmpty) Stream.empty
            ...
            Stream(JDBCWriteOp(JDBCUpdate(name, columnExpressions(updateColumns), whereClauses), config, binder))
          }
        }
    }

    override def deleteAll: Pipe[JDBCIO, Data, WriteOp] = _.map(t => deleteWriteOp(toKey(allColumns.iso.to(t))))
  ...
  def select = new QueryBuilder[C, DataRec, BindNone.type, HNil, HNil](this, ColumnRecord.empty, identity,
    BindNone, Seq.empty)

  def select[ColNames <: HList, Out <: HList](cols: Cols[ColNames])
                               (implicit css: ColumnSubsetBuilder.Aux[DataRec, ColNames, Out])
  = new QueryBuilder[C, DataRec, BindNone.type, HNil, HNil](this, ColumnRecord.empty, identity,
    BindNone, Seq.empty).cols[Out, Out, ColNames](cols)

  def delete = new DeleteBuilder[C, DataRec, BindNone.type](this, BindNone)

  def query = new QueryBuilder[C, DataRec, BindNone.type, DataRec, Data](this, toProjection(allColumns),
    allColumns.iso.from, BindNone, Seq.empty)
  ...
  def definition: TableDefinition = TableDefinition(name, allColumns.columns.map(NamedColumn.apply[C]),
    keyColumns.columns.map(_._1))

  def cols[Names <: HList](c: Cols[Names])(implicit ss: ColumnSubsetBuilder[DataRec, Names]): ColumnSubset[C, DataRec, ss.Out, ss.Out] =
    allColumns.subset[Names]._1

  def subset[Names <: HList](c: Cols[Names])(implicit ss: ColumnSubsetBuilder[DataRec, Names]): TableColumns =
...
object JDBCTable
{
  ...
  def apply[C[_] <: JDBCColumn, T, R <: HList, K <: HList, KeyN <: HList](tableName: String, jdbcConfig: JDBCConfig,
                                               all: Columns[C, T, R], keys: ColumnSubsetBuilder.Aux[R, KeyN, K],
                                               tkey: R => K): Aux[C, T, R, K, KeyN] = new JDBCTable[C] {
