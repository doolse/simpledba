...
package object sqlserver {
  ...
  case class SQLServerColumn[A](wrapped: StdJDBCColumn[A], columnType: ColumnType) extends WrappedColumn[A]
  ...
  trait StdSQLServerColumns extends StdColumns
  {
    ...
    implicit def stringCol = SQLServerColumn[String](StdJDBCColumn.stringCol, ColumnType("NVARCHAR(MAX)"))

    implicit def boolCol = SQLServerColumn[Boolean](StdJDBCColumn.boolCol, ColumnType("BIT"))

    implicit def instantCol = SQLServerColumn[Instant](StdJDBCColumn.instantCol, ColumnType("DATETIME"))
    ...
    override def wrap[A, B](col: SQLServerColumn[A],
                            edit: StdJDBCColumn[A] => StdJDBCColumn[B],
                            editType: ColumnType => ColumnType): SQLServerColumn[B] =
  ...
  def sqlServerTypeNames(stringKeySize: Int)(b: ColumnType, key: Boolean): String = {
    val baseType = if (key && b.typeName == "NVARCHAR(MAX)") s"NVARCHAR($stringKeySize)" else b.typeName
    if (b.hasFlag(IdentityColumn)) {
      s"$baseType IDENTITY"
    } else baseType
  }

  def sqlServerConfig = JDBCSQLConfig[SQLServerColumn](defaultEscapeReserved,
    defaultEscapeReserved, stdSQLQueries, stdExpressionSQL, sqlServerTypeNames(256), SQLServerSQL.apply)
  ...
  def insertIdentity[C[_] <: JDBCColumn, T, R <: HList,
  KeyNames <: HList,
  AllCols <: HList,
  WithoutKeys <: HList,
  JustKeys <: HList, A,
  Res](table: JDBCTable.Aux[C, T, R, A :: HNil, KeyNames])(
